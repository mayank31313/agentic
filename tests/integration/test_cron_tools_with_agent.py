...
def test_cron_create_tool(agent, judge):
    ...
    final_messages = list(filter(lambda x: x['type'] == 'text', result['messages'][-1].content))
...
def test_cron_update_tool(agent, judge):
...
def test_cron_list_tool(agent, judge):
    cron_schedule = CronSchedule.model_validate({
        "cron_expression": "0 21 * * *",
        "id": "0aa95f00-3967-4ce0-8318-8d70f80b0056",
        "name": "cron_schedule",
