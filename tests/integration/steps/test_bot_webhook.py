...
def connect_to_bot(websocket, judge):
    ...
    message = OutboundMessage.model_validate(json.loads(response))
    ...
    judge_response  = judge.invoke(dict(user_query=user_message, response=response_text, criterias="Response is a casual greeting"), timeout=10)
