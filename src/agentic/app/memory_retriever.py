...
class MemoryRetriever:
    ...
    def __init__(self, 
        ...
        if not ELASTICSEARCH_AVAILABLE and enabled:
            logger.warning("Elasticsearch not available. Using in-memory fallback. Install: pip install elasticsearch")
            self.enabled = False
    ...
    def _init_elasticsearch(self):
        ...
        try:
            ...
            if not self.es_client.ping():
                logger.warning(f"Cannot connect to Elasticsearch at {self.es_host}:{self.es_port}")
                self.enabled = False
                return
            ...
            logger.warning(f"Failed to initialize Elasticsearch: {e}. Using in-memory fallback.")
            self.enabled = False
    
    def add_message(self, chat_id: int, role: str, content: str, message_index: int = 0):
        """
    ...
    def _es_topic_search(self, chat_id: int, keywords: List[str], limit: int) -> List[Dict]:
    ...
    def _in_memory_topic_search(self, chat_id: int, keywords: List[str], limit: int) -> List[Dict]:
    ...
    async def get_chat_history(self, chat_id: int, limit: int = 10) -> List[Dict]:
        ...
        try:
            if self.enabled:
                results = await asyncio.wait_for(
                    asyncio.to_thread(self._es_get_history, chat_id, limit),
                    timeout=5.0
                )
                return results
            else:
...
def get_memory_retriever(es_host: str = "localhost",
                        es_port: int = 9200,
                        enabled: bool = True) -> MemoryRetriever:
