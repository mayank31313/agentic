...
@Bean()
def getBankgroundScheduler(agenticConfig: AgenticConfig, tool_registry: ToolsRegistry) -> BackgroundScheduler:
        ...
        try:
            ...
            if action != "DELETE":
                scheduler.add_job(execute_task,
                                  CronTrigger.from_crontab(cron.cron_expression), id=str(cron.id),
    ...
    def execute_task(cron: CronSchedule):
        ...
        tools = list(filter(lambda x: x.name not in agent_config.denied_tools, tool_registry.tools.values()))
    ...
    with TinyDB(os.environ.get('CRON_SCHEDULE_FILENAME', "cron_schedules.json")) as db:
        ...
        for cron in crons:
            scheduler.add_job(execute_task, CronTrigger.from_crontab(cron.cron_expression), kwargs=dict(cron=cron))
